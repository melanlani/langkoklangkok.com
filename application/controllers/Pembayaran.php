...
class Pembayaran extends CI_Controller
{
	...
	function detailpembayaran()
	{
		...
		{	
			...
			$this->load->view('member/tagihanview',$d);
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			if($this->m_db->is_bof('penjualan_konfirmasi',$s)==FALSE)
			{
				...
				if($this->m_db->add_row('penjualan_konfirmasi',$d)==TRUE)
				{
					...
					redirect(site_url().'/pembayaran/konfirmasi?s=1');
				}
			}
		}else{
			$this->load->view('member/konfirmasiview');
		}
	}
	...
	function getinvoice()
	{
		...
		if($this->m_db->is_bof('penjualan',$s)==FALSE)
		{
			$nama=field_value('pelanggan','pelanggan_id',$pelanggan->pelanggan_id,'nama_pelanggan');
	...
	function cektagihan()
	{
		...
		$dx['data']=$d;
		$this->load->view('member/trackingview',$dx);
	}
}