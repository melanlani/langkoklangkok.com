...
if(!function_exists('resep_masakan'))
{
	function resep_masakan($produkID,$limit=null)
	{
		$CI=& get_instance();
		$s=array(
		'id_resep'=>$produkID,
		);
		$d=$CI->m_db->get_data('resep_masakan',$s,array(),array(),$limit);
		return $d;
	}
}
...
if(!function_exists('resep_kategori'))
{
	function resep_kategori()
	{
		$CI=& get_instance();
		$CI->load->model('resep_model');
		$d=$CI->resep_model->kategori_data();
		return $d;
	}
}
...
if(!function_exists('resep_kategori_data'))
{
	function resep_kategori_data($kategori,$limit)
	{
		$CI=& get_instance();
		$s=array(
		'id_masakan'=>$kategori,
		);
		$d=$CI->m_db->get_data('resep_masakan',$s,'id_resep DESC',array(),$limit);
		return $d;
	}
}
if(!function_exists('produk_supplier_data'))
{
	function produk_supplier_data($supplier,$limit)
	{
		$CI=& get_instance();
		$s=array(
...
if(!function_exists('resep_bumbu_data'))
{
	function resep_bumbu_data($bumbu,$limit)
	{
		$CI=& get_instance();
		$s=array(
		'id_resep'=>$bumbu,
		);
		$data=$CI->m_db->get_data('produk',$s,'produk_id DESC',array(),$limit);
		return $data;
	}
}
...
if(!function_exists('supplier_info'))
{
	function supplier_info($output='supplier_id')
	{
		$CI=& get_instance();
		$o="";
		if(roleid()=="3")
		{
			...
			$o=$CI->m_db->get_row('supplier',$s,$output);
		}
		return $o;
	}
}
