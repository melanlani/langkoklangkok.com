...
$pembelian=$this->m_db->get_data('penjualan');
if(empty($pembelian))
{
                    ...
                    <div class="panel panel-info">
						...
						<div class="form-horizontal">
							...
							<div class="col-xs-4">
								...
								<?php if($pelangganid == 0){?>
								<div class="form-group">
									<label class="col-xs-3 control-label" for="">Kepada</label>
									<div class="col-xs-9">
										<p class="form-control-static"><?=$row->tamu;;?></p>
									</div>
								</div>
								<?php }else{ ?>
								<div class="form-group">
							...
							<div class="col-xs-4">
								...
								<div class="form-group">
									...
									<div class="col-xs-9">
										<p class="form-control-static"><?=$row->notelp;?></p>
									</div>
								</div>
								<?php }else{ ?>
								<div class="form-group">
								...
								<tbody>
									...
									if(!empty($data))
									{
										...
										<tr>
											...
											<td>
												<?=produk_info($item->produk_id,'kode_produk');?>-<?=produk_info($item->produk_id,'nama_produk');?><br/>
												Berat : <?=produk_info($item->produk_id,'berat');?> gram
											</td>
											<td>
												<?=$item->qty;?>
											</td>
						...
						echo form_open(site_url('order/gudang_produk'),array('class'=>'form-horizontal'));
						?>
						<?php if(!empty($data))
									{
										foreach($data as $item)
						...
						<?php }}?>
						<?php $jual=$this->uri->segment(4); ?>
						<input type="hidden" name="penjualan_id" value="<?=$jual;?>"/>
						<input type="hidden" name="userid" value="<?php echo $this->session->userdata('userid'); ?>"/>
						<div class="form-group">
							<label class="col-sm-2 control-label">&nbsp;</label>
							<div class="col-md-10">
								<button type="submit" onclick="return confirm('Yakin telah menempatkan produk di gudang?');" class="btn btn-primary btn-flat">Saya telah menempatkannya digudang</button>
