                            ...
                            <tbody>
								...
								?>
										...
										<td>
											...
											{
												?>
												<a onclick="return confirm('Yakin ingin menghapus order ini?');" href="<?=site_url();?>/order/delete?id=<?=$row->penjualan_id;?>" class="btn btn-danger btn-xs btn-flat"><i class="fa fa-trash"></i></a>
												<?php
											}elseif($row->status=="konfirmasi"){
												?>
												<a href="<?=site_url();?>/order/approve?id=<?=$row->penjualan_id;?>" class="btn btn-success btn-xs btn-flat"><i class="fa fa-money"></i> Validasi</a>
												<?php
											}elseif($row->status=="lunas"){
												
												?>
												<a href="<?=site_url();?>/order/shipping?id=<?=$row->penjualan_id;?>" class="btn btn-success btn-xs btn-flat"><i class="fa fa-truck"></i> Validasi</a>
										...
										$take=$this->m_db->get_data('penjualan_detail',array('penjualan_id'=>$row->penjualan_id));
												if(!empty($take))
												{
													foreach($take as $rOrder)
													{
														...
														{
															?>
															<li><?=$nama_produk;?> Sudah digudang</li>
															<?php } elseif($rOrder->gudang=="belum")
															{
																?>
																<li><?=$nama_produk;?> belum digudang</li>
																<?php
																}}}?>
										</td>
									</tr>
